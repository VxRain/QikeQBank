<template>
  <div class="page">
    ...
    <div v-if="!stats && !error" class="hint">正在加载统计…</div>
    ...
    <template v-else-if="stats">
      ...
      <div class="stat-grid">
        ...
        <div class="card stat">
          <div class="num review-num">{{ stats.due_total ?? 0 }}</div>
          <div class="lbl">待复习总数</div>
          <div class="sub">今日到期 {{ stats.due_today ?? 0 }}</div>
        </div>
        <div class="card stat">
          <div class="num">{{ stats.practiced_total ?? 0 }}</div>
          <div class="lbl">累计刷题</div>
          <div class="sub">今日 {{ stats.practiced_today ?? 0 }}</div>
        </div>
        <div class="card stat">
          <div class="num">{{ rateText }}</div>
      ...
      <div class="entry-grid">
        ...
        <router-link to="/library" class="card entry">
          ...
          <div class="entry-desc">浏览 / 编辑 / 删除试题</div>
        </router-link>
        <router-link to="/create" class="card entry">
          <div class="entry-title">新增试题</div>
          <div class="entry-desc">创建并保存新题目</div>
        </router-link>
...
import { stats as fetchStats, exportData } from '@/api/practice.js'

const TYPE_ORDER = ['single', 'multi', 'judge', 'fill', 'short', 'material']
const TYPE_LABELS = {
  single: '单选', multi: '多选', judge: '判断', fill: '填空', short: '简答', material: '材料'
}
...
.hint { color: var(--muted); padding: 24px 0; }
.error {
...
.entry:hover {
  ...
  border-color: var(--primary-border); background: var(--card);
}

#!/usr/bin/env node
/**
 * W4 · tools/migrate-dbjson.mjs
 * 将 QBank Web 版 DB.json 迁移为 SQLite 库。
 * 用法:
 *   node tools/migrate-dbjson.mjs <input-DB.json> <output.db> [--dry-run]
 *
 * 流程: 参数校验 → 打开/创建 output.db → 执行 PLAN.md §3 DDL（逐字符一致）
 *       → 读 input.questions 数组 → 逐题 INSERT → 打印总题数/各 type 计数/写入路径。
 *
...
/** PLAN.md §3 SQLite 表结构 —— 唯一权威 DDL，禁止与 PLAN 有任何字符差异 */
...
function buildInsert(q, id, now) {
  ...
  const cols = ['id', 'type', 'stem_json', 'options_json', 'answer_json', 'analysis_json',
...
function main() {
  ...
  db.close();
  process.exit(dryRun || failed === 0 ? 0 : 1);
}
